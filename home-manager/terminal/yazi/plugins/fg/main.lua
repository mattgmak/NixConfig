...
local get_option = ya.sync(function(self) return self.active_opt end)
...
local function splitAndGetNth(inputstr, sep, index)
  ...
  while true do
    ...
    count = count + 1
    if count == index then return string.sub(inputstr, start, sepStart - 1) end
    start = sepEnd + 1
  end
...
function M:entry(job)
  ...
  if output.stdout == '' then return end
...
function M:setup(config) self.default_action = (config and config.default_action) and config.default_action or 'menu' end
