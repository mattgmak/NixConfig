# Neovide-style cursor trail; trail color from Stylix base0E (magenta/pink).
{ ... }:
{
  flake.cursorCustomJs =
    colors:
    let
      pink = colors.withHashtag.base0E;
    in
    ''
      {
        ...
        // Trail / cursor accent (Stylix base16 base0E). Use "default" for CSS var from theme.
        const Color = "${pink}";
        ...
        function createTrail(options) {
          ...
          let particlesColor = options?.color || "${pink}";
